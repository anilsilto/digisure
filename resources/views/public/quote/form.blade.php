...
@section('content')
    <section class="mx-auto max-w-3xl px-4 py-14">
        ...
        @if ($errors->any())
            <div class="mt-6 rounded-lg border border-danger/30 bg-danger-tint px-4 py-3 text-sm text-danger">
        ...
        <form method="POST" action="{{ route('teklif.store') }}" class="mt-8 space-y-8">
                ...
                @foreach ($selected?->field_schema ?? [] as $field)
                    ...
                    <div>
                        <label class="block text-sm font-medium text-ink">
                            ...
                            @if ($field['required'] ?? false)<span class="text-accent">*</span>@endif
                        </label>
            ...
            <div class="space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-medium text-ink">Ad <span class="text-accent">*</span></label>
                        <input type="text" name="ad" value="{{ old('ad') }}"
                               class="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-navy focus:outline-none">
                        @error('ad') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-ink">Soyad <span class="text-accent">*</span></label>
                        <input type="text" name="soyad" value="{{ old('soyad') }}"
                               class="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-navy focus:outline-none">
                        @error('soyad') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-ink">TC Kimlik No <span class="text-accent">*</span></label>
                        <input type="text" name="tc_no" value="{{ old('tc_no') }}" maxlength="11" inputmode="numeric"
                               class="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-navy focus:outline-none">
                        @error('tc_no') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-ink">Telefon <span class="text-accent">*</span></label>
                        <input type="tel" name="telefon" value="{{ old('telefon') }}"
                               class="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-navy focus:outline-none">
                        @error('telefon') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-ink">E-posta</label>
                        <input type="email" name="eposta" value="{{ old('eposta') }}"
                               class="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-navy focus:outline-none">
                        @error('eposta') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div>
                <label class="flex items-start gap-2 text-sm text-muted">
                    ...
                    <span>
                        ...
                        okudum, kişisel verilerimin teklif ve poliçe süreçleri için işlenmesini kabul ediyorum. <span class="text-accent">*</span>
                    </span>
                </label>
                @error('kvkk') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
            </div>

            <button type="submit"
                    class="rounded-lg bg-accent px-6 py-3 text-sm font-semibold text-white transition hover:bg-accent-dark">
